...
@end`;
  ...
  Active
@end`;
        expect(engine.render(template, { status: 'active' })).toContain(
          'Active',
        );
        expect(engine.render(template, { status: 'inactive' })).not.toContain(
          'Active',
        );
      });
...
@end`;
        ...
        expect(engine.render(template, { count: 4 })).not.toContain(
          'Five or more',
        );
      });
...
@end`;
        ...
        expect(engine.render(template, { count: 6 })).not.toContain(
          'Five or less',
        );
      });
    });
...
@end`;
  ...
  Visible
@end`;
        expect(engine.render(template, { isDeleted: false })).toContain(
          'Visible',
        );
        expect(engine.render(template, { isDeleted: true })).not.toContain(
          'Visible',
        );
      });
...
@elif(status == "published")
  Published
@end`;
        expect(engine.render(template, { status: 'published' })).toContain(
          'Published',
        );
        expect(engine.render(template, { status: 'published' })).not.toContain(
          'Draft',
        );
      });
...
@else
  Unknown
@end`;
        expect(engine.render(template, { status: 'archived' })).toContain(
          'Unknown',
        );
        expect(engine.render(template, { status: 'archived' })).not.toContain(
          'Draft',
        );
        expect(engine.render(template, { status: 'archived' })).not.toContain(
          'Published',
...
@else
  Outer False
@end`;
        expect(engine.render(template, { outer: true, inner: 1 })).toContain(
          'Inner One',
        );
        expect(engine.render(template, { outer: true, inner: 2 })).toContain(
          'Inner Two',
        );
        expect(engine.render(template, { outer: true, inner: 3 })).toContain(
          'Inner Other',
        );
        expect(engine.render(template, { outer: false, inner: 1 })).toContain(
          'Outer False',
        );
      });
    });
...
@end`;
        expect(
          engine.render(template, { show: false, message: 'Secret' }),
