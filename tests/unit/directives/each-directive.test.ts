...
describe('Each Directive', () => {
    describe('parseArguments', () => {
        ...
        it('should throw on missing "in" keyword', () => {
            expect(() => directive.parseArguments('item items')).toThrow(APTLSyntaxError);
            expect(() => directive.parseArguments('item items')).toThrow(/missing 'in' keyword/i);
        });

        it('should throw on missing item name', () => {
            expect(() => directive.parseArguments('in items')).toThrow(APTLSyntaxError);
        });

        it('should throw on missing array path', () => {
            expect(() => directive.parseArguments('item in')).toThrow(APTLSyntaxError);
        });
    });
    ...
    describe('validate', () => {
        ...
        it('should throw if no arguments provided', () => {
            ...
            expect(() => directive.validate(node)).toThrow('requires iteration syntax');
        });
        ...
        it('should throw on invalid iteration syntax', () => {
            ...
            expect(() => directive.validate(node)).toThrow(APTLSyntaxError);
            expect(() => directive.validate(node)).toThrow('Invalid iteration syntax');
        });
    });
    ...
    describe('execute', () => {
                    ...
                    return childrenToRender
                        .map((child: any) => {
                            if (child.type === NodeType.TEXT) {
                                ...
                                const value = path.split('.').reduce((obj: any, key: string) => obj?.[key], renderData);
        ...
        it('should iterate over array and render content for each item', () => {
            ...
            const context = createMockContext(node, { items: ['apple', 'banana', 'cherry'] });
        ...
        it('should provide loop.even and loop.odd flags', () => {
            ...
            expect(result).toBe('0:even=true,odd=false 1:even=false,odd=true 2:even=true,odd=false ');
        });
        ...
        it('should render nothing for empty array', () => {
            const node: DirectiveNode = {
                ...
                rawArgs: 'item in items',
                children: [
                    createTextNode('Should not see this'),
                ],
                line: 1,
                column: 1,
            };
        ...
        it('should throw if renderTemplate is not available', () => {
            ...
            expect(() => directive.execute(context)).toThrow('requires renderTemplate');
        });
    });
    ...
    describe('@else branch', () => {
        ...
        it('should render else branch with complex content', () => {
            ...
            const context = createMockContext(node, { items: [], itemType: 'products' });
            const result = directive.execute(context);
                    ...
                    return childrenToRender
                        .map((child: any) => {
                            if (child.type === NodeType.TEXT) {
                                ...
                                const value = path.split('.').reduce((obj: any, key: string) => obj?.[key], renderData);
                                return value != null ? String(value) : '';
                            } else if (child.type === NodeType.DIRECTIVE && child.name === 'each') {
