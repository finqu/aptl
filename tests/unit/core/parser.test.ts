...
describe('Parser - The AST Architect', () => {
    ...
    describe('Variables - The Interpolation Station', () => {
        ...
        it('should parse multiple variables', () => {
            ...
            const variables = ast.children.filter((c) => c.type === NodeType.VARIABLE);
...
@end`;
        ...
        it('should parse directive with variables in body', () => {
            ...
            const variable = directive.children.find((c: any) => c.type === NodeType.VARIABLE);
        ...
        it('should parse directive with string literals in args', () => {
            const ast = parseTemplate('@section test(format="json", lang="js")\nCode\n@end\n');
            const directive = ast.children[0] as any;
...
@end`;
            const ast = parseTemplate(template);
            const directive = ast.children[0] as any;
            const textNodes = directive.children.filter((c: any) => c.type === NodeType.TEXT);
    ...
    describe('Edge Cases - The Corner Dwellers', () => {
        ...
        it('should handle template with only variables', () => {
            ...
            expect(ast.children).toHaveLength(3);
            expect(ast.children.every((c) => c.type === NodeType.VARIABLE)).toBe(true);
        });
...
@end`;
            ...
            while (current && current.type === NodeType.DIRECTIVE) {
                depth++;
                current = current.children.find((c: any) => c.type === NodeType.DIRECTIVE);
            }
            expect(depth).toBe(4);
        });
...
@end`;
            ...
            const mainSection = ast.children.find((c: any) =>
                c.type === NodeType.DIRECTIVE && c.name === 'section'
            ) as any;
...
Final text`;
            ...
            const hasVariable = ast.children.some((c) => c.type === NodeType.VARIABLE);
            const hasDirective = ast.children.some((c) => c.type === NodeType.DIRECTIVE);
