 ...
 */
export function isClassBasedDirective(
  directive: DirectiveType,
): directive is BaseDirective {
...
export class DirectiveRegistry {
  ...
  register(directive: DirectiveType): void {
    const name = isClassBasedDirective(directive)
      ? directive.name
      : directive.name;
  ...
  getAllKeywords(): string[] {
    ...
    for (const directive of this.directives.values()) {
      if (
        isClassBasedDirective(directive) &&
        'getConditionalKeywords' in directive
      ) {
